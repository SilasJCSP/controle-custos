...
def salvar_transacoes(df: pd.DataFrame) -> int:
    ...
    colunas = ["id_transacao", "data", "descricao", "valor", "categoria", "banco", "arquivo_fatura", "origem", "tipo_lancamento"]
...
def registrar_importacao(arquivo: str, hash_arquivo: str, banco: str, qtd_transacoes: int, status: str, erro: str = "") -> None:
    init_db()
    with connection() as conn:
        conn.execute(
            """
            INSERT INTO importacoes (arquivo, hash_arquivo, banco, data_importacao, qtd_transacoes, status, erro)
            VALUES (?, ?, ?, CURRENT_TIMESTAMP, ?, ?, ?)
            """,
            (arquivo, hash_arquivo, banco, int(qtd_transacoes), status, erro),
        )


def arquivo_ja_processado(hash_arquivo: str) -> bool:
    if not hash_arquivo:
        return False
    init_db()
    with connection() as conn:
        rows = conn.execute("SELECT 1 FROM importacoes WHERE hash_arquivo = ? LIMIT 1", (hash_arquivo,)).fetchone()
...
def salvar_email_processado(gmail_id: str, thread_id: str = "", remetente: str = "", assunto: str = "", mensagem_id: str = "", arquivo_pdf: str = "", hash_conteudo: str = "") -> None:
    init_db()
