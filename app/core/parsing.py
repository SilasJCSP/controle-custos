...
from app.core.transactions import deduplicar_transacoes
...
def _extrair_transacao_linha(linha: str, contexto: ParseContext) -> dict[str, object] | None:
    ...
    return {"data": data.date(), "descricao": descricao, "valor": valor, "banco": contexto.banco, "arquivo_fatura": contexto.arquivo_fatura}


def extrair_transacoes_basicas(linhas: Iterable[str], contexto: ParseContext) -> pd.DataFrame:
    ...
    logger.info("%s [%s]: %d linhas convertidas, %d descartadas", contexto.arquivo_fatura, contexto.banco, linhas_convertidas, linhas_descartadas)

    if not transacoes:
        return pd.DataFrame(columns=["data", "descricao", "valor", "banco", "arquivo_fatura"])

    df = pd.DataFrame(transacoes)
    return deduplicar_transacoes(df)
